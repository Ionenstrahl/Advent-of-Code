...
let correctedGalaxies;
setCorrectedGalaxies(1000000);

export function setCorrectedGalaxies(expansion) {
    correctedGalaxies = galaxies
        .map(galaxy => {
            galaxy.x += xShift(galaxy, expansion);
            galaxy.y += yShift(galaxy, expansion);
            return galaxy
        })
}

function xShift(galaxy: Galaxy, expansion: number) {
    return emptyColumns.filter(col => col < galaxy.x).length * (expansion - 1);
}

function yShift(galaxy: Galaxy, expansion: number) {
    return emptyRows.filter(row => row < galaxy.y).length * (expansion - 1);

}

export function distanceSum() {
